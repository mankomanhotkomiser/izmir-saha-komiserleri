          ...
          <div id="gorev-karti-alani" className="bg-slate-200 min-h-full">
            <div className="bg-white p-4 rounded-xl shadow-sm mb-5 border-b-2 border-blue-900 flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                <button onClick={() => setTebellugEdildi(true)} disabled={tebellugEdildi || aktifMaclar.length === 0} className={`text-sm font-bold py-2 px-4 rounded-lg shadow flex items-center gap-2 transition-colors ${tebellugEdildi ? 'bg-slate-200 text-slate-500 cursor-not-allowed border border-slate-300' : aktifMaclar.length > 0 ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-slate-300 text-slate-500 cursor-not-allowed'}`}>
                  {tebellugEdildi ? '✓ Tebellüğ Edildi' : 'Tebellüğ Et (Görevleri Aldım)'}
                </button>
            ...
            <div className="p-6 space-y-8">
              ...
              <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-4 transition-colors hover:bg-red-100">
                ...
                <label htmlFor="kompleYokum" className="cursor-pointer">
                  ...
                  <span className="block text-red-500 text-sm mt-1">İşaretlerseniz detaylı gün ve saat seçimleri pasif hale gelir.</span>
                </label>
              </div>

              <div className={`transition-opacity duration-300 space-y-6 ${kompleYokum ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
                
                <div>
                  <h3 className="font-bold text-slate-700 border-b pb-2 mb-4 uppercase tracking-wider">Hafta İçi (Cum-Per)</h3>
                  
                  <label className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg border border-slate-300 cursor-pointer hover:bg-slate-100 transition-colors mb-3">
                    <input 
                      type="checkbox" 
                      checked={haftaIciMusait}
                      onChange={(e) => setHaftaIciMusait(e.target.checked)}
                      className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500 cursor-pointer"
                    />
                    <span className="font-bold text-lg text-slate-800">Hafta İçi Müsaitim</span>
                  </label>
                ...
                <div>
                  ...
                  <label className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg border border-slate-300 cursor-pointer hover:bg-slate-100 transition-colors mb-3">
                    <input 
                      ...
                      onChange={(e) => setHaftaSonuMusait(e.target.checked)}
                      className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500 cursor-pointer"
                    />
                    <span className="font-bold text-lg text-slate-800">Hafta Sonu Müsaitim</span>
                  </label>
              ...
              <button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-lg transition-transform transform hover:scale-[1.01] active:scale-95 text-lg flex items-center justify-center gap-2 mt-4">
