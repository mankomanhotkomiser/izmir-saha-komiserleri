...
export default function AdminPage() {
  ...
  // -------------------------------------------------------------
  // ORTAK TFF RAPORU ÇİZİM ŞABLONU
  // -------------------------------------------------------------
  const renderTffRaporu = (mac: any, prefix: string) => {
      ...
      // KOMİSERİN TELEFON NUMARASI
      const ilgiliKomiser = tumKomiserler.find(k => String(k.komiser_id) === String(mac.komiser_id));
      const komiserTelefon = ilgiliKomiser ? ilgiliKomiser.telefon : '';
      ...
      return (
          <div id={`${prefix}-tff-form-${mac.id}`} className="min-w-[700px] max-w-4xl w-full bg-white p-6 border-2 border-black relative font-sans text-black shadow-sm mx-auto flex flex-col gap-6">
  ...
  // DEĞİŞKENLER - SADECE BİR KERE TANIMLANIYOR!
      ...
      <div className={`mb-3 rounded-xl border-l-4 overflow-hidden shadow-md transition-all ${renkSiniflari.border}`}>
        <button onClick={() => toggleMac(mac.id)} className={`w-full text-left p-4 flex justify-between items-center ${renkSiniflari.bg} hover:brightness-125 transition-all focus:outline-none`}>
            ...
            <div className="flex flex-col items-end pl-2 gap-1.5 mt-2 sm:mt-0">
                ...
                {raporTurunuBelirle(mac.kategori_adi) !== 'yok' && tip !== 'tebellug' && (
                            ...
                            <div className="bg-slate-800/50 border border-slate-600 rounded-lg overflow-hidden">
                                ...
                                {isTffAcik && (
                                    <div className="p-4 overflow-x-auto bg-slate-300">
                                        {renderTffRaporu(mac, 'admin')}
                                        <div className="mt-4 flex justify-end">
                                            <button onClick={() => tffTutanakIndir(mac, 'admin')} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition-all text-xs md:text-sm flex items-center justify-center gap-2">
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <div className="space-y-6">
                                            ...
                                            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 relative overflow-hidden shadow-md mt-6">
                                                <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500"></div>
                                                <h4 className="text-emerald-400 font-bold text-sm tracking-wider uppercase mb-3 flex items-center gap-2 border-b border-slate-700 pb-2"><span className="text-lg">🏟️</span> GÖREV YAPILAN SAHALAR VE DETAYLI ARŞİV</h4>
                                                <div className="space-y-3 max-h-[700px] overflow-y-auto custom-scrollbar pr-2">
                                                    ...
                                                    {siraliSahalarDetayli.map(([saha, data]) => {
                                                        ...
                                                        return (
                                                            <div key={saha} className="bg-slate-900 border border-slate-700 rounded-lg overflow-hidden shadow-sm transition-all">
                                                                <button onClick={() => setAcikSicilSaha(isSahaAcik ? null : saha)} className="w-full text-left p-3 flex justify-between items-center hover:bg-slate-700/50 transition-colors focus:outline-none">
                                                                    <div>
                                                                        <span className="text-slate-200 font-bold block">{saha}</span>
                                                                        <span className="text-[10px] text-emerald-400 font-mono mt-1 block bg-emerald-900/30 px-2 py-0.5 rounded border border-emerald-800/50 inline-block">{gidisSayisi} Farklı Gün (Toplam {gorevSayisi} Görev)</span>
                                                                    </div>
                                                                    <span className="text-slate-500 text-lg leading-none">{isSahaAcik ? '▲' : '▼'}</span>
                                                                </button>
                                                                
                                                                {isSahaAcik && (
                                                                    <div className="p-3 bg-slate-800 border-t border-slate-700 space-y-3 animate-fade-in-down">
                                                                        {data.maclar.sort((a,b) => getZaman(b) - getZaman(a)).map((mac, idx) => {
                                                                            ...
                                                                            return (
                                                                                <div key={mac.id || idx} className="bg-slate-900 border border-slate-600 rounded p-3 text-xs text-slate-300">
                                                                                    <div className="flex justify-between items-start pb-2 mb-2 gap-2">
                                                                                        <div className="flex-1">
                                                                                            <div className="text-emerald-500 font-bold text-[9px] mb-1 tracking-widest">{mac.kategori_adi}</div>
                                                                                            <div className="font-bold text-sm text-white mb-1">{mac.ev_sahibi} <span className="text-slate-500 mx-1 text-[10px]">vs</span> {mac.misafir_takim}</div>
                                                                                            <div className="text-[10px] text-slate-400">{guvenliTarih(mac.tarih)} - {guvenliSaat(mac.saat)}</div>
                                                                                        </div>
                                                                                        <div className="flex flex-col gap-2 items-end">
                                                                                            <div className="bg-slate-700 text-white font-black px-3 py-1.5 rounded shadow-inner border border-slate-500 text-center min-w-[70px]">
                                                                                                {skorMetni}
                                                                                            </div>
                                                                                            {detayliGonderilmis && (
                                                                                                <button onClick={() => setAcikSicilTffMacId(isSicilTffAcik ? null : mac.id)} className="bg-blue-900/50 hover:bg-blue-800 text-blue-300 border border-blue-700/50 px-2 py-1 rounded text-[9px] font-bold uppercase transition-colors flex items-center gap-1">
                                                                                    ...
                                                                                    {isSicilTffAcik && detayliGonderilmis && (
                                                                                        <div className="mt-4 border-t border-slate-700 pt-4 animate-fade-in-down">
                                                                                            <div className="p-4 overflow-x-auto bg-slate-300 rounded-lg">
                                                                                                {renderTffRaporu(mac, 'sicil')}
                                                                                            </div>
                                                                                            <div className="mt-4 flex justify-end">
                                                                                                <button onClick={() => tffTutanakIndir(mac, 'sicil')} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition-all text-xs md:text-sm flex items-center justify-center gap-2">
                                                                                                    📸 FOTOĞRAF (PNG) OLARAK İNDİR
                                                                                                </button>
                                                                                            </div>
                                                                                        </div>
